...
uint8_t nrf24l01_write(uint8_t *data) {
	uint8_t i = 0;
	uint8_t ret = 0;
	//uint8_t dataaux[NRF24L01_PAYLOAD];
	//memcpy(dataaux,data,NRF24L01_PAYLOAD);
	//set tx mode
	nrf24l01_setTX();
...
void nrf24l01_init() {
	...
	NRF24L01_CE_DDR |= (1<<NRF24L01_CE); //output |= (1<<NRF24L01_CE); //output
	...
	nrf24l01_writeregister(NRF24L01_REG_FEATURE, 4); //disable dynamic payloads  //TEST
...
uint8_t nrf24l01_writeRT() {
	...
	for (i=0; i<NRF24L01_PAYLOAD; i++)               //TU=

	spi_writereadbyte(maxRt);
	...
	while( !(nrf24l01_getstatus() & 1<<NRF24L01_REG_TX_DS) && !maxRt==75);
